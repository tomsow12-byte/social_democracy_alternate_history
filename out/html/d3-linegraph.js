...
d3.linegraph = function(noTicks, noDots, parties, partyColors, partyNames, dataMax, dataMin, additionalMonths) {
    /* params */
    if (!parties) {
        parties = ['razem', 'lewica', 'pl2050', 'po', 'psl', 'konfa', 'korona', 'other'];
    }
    if (!partyColors) {
        partyColors = {'razem': '#E3000F', 'lewica': '#8B0000', 'pl2050': '#DCCA4A', 'po': '#000', 'psl': '#D5AC27', 'konfa': '#3f7bc1', 'korona': '#954B00', 'other': '#a0a0a0'};
    }
    if (!partyNames) {
        partyNames = {'razem': 'Razem', 'lewica': 'Lewica', 'pl2050': 'PL2050', 'po': 'PO', 'psl': 'PSL', 'konfa': 'Konfederacja', 'korona': 'KKP', 'other': 'Others'};
    }
    if (!additionalMonths) {
        additionalMonths = 10;
    }
    ...
    function linegraph(dataset) {
     dataset.each(function (data) {
      ...
      if (!dataMax) {
          const maxRazem = d3.max(data, d => d.razem);
          const maxNSDAP = d3.max(data, d => d.korona);
          dataMax = maxRazem >= maxNSDAP ? maxRazem + 10 : maxNSDAP + 10;
          dataMin = 0;
      }
